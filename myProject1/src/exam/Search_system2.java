...
public class Search_system2 extends DbConnect {
	...
	public static void main(String[] args) throws Exception {
	 ...
	 public static void ExamScoreSearch() throws Exception {
		 ...
		 System.out.print("학년을 입력해주세요>>");
  	     ...
  	     String sql = "select * from (select b.*,rank() over(order by sum desc) banrank from( select a.*,rank() over(order by sum desc) totalrank from("
			  	    	   +" select i.userid as userid, i.name as name, i.grade as grade, e.ban as ban, e.hackgi as hackgi, e.eng as eng"
			  	    	   		+" , e.kor as kor, e.math as math,(e.eng+e.kor+e.math) as sum ,round((e.eng+e.kor+e.math)/3,0) as avg"
			  	    	   			+" from student_info i,exam_score e where i.name = e.name and i.grade = e.grade and e.grade = " + clValue2 + " and e.hackgi = " + clValue3
			  	                   		+" ) a ) b where ban = (select ban from exam_score e where " + where + ")) where name = '"+clValue+"' ";
//  	     System.out.println(sql);
		...
		while(rs.next()) {
			...
			int avg = sum/3;
			int banrank = rs.getInt("banrank");
			int totalrank = rs.getInt("totalrank");
