...
public class EmpAdmin extends DbConnect {
	...
	public static void dnameSearch() throws Exception{
		...
		while(rs.next()) {
			...
			String sibal = (rownum.equals("1") || rownum.equals("6")) ? "   " : "";
			System.out.printf("%-5s %-18s %-15s %s%n", rownum,ename,job,dname1);
		}
		if (cnt==0) {
			System.out.println("***검색 결과가 없습니다***");
		}
	...
	public static void mgrnameSearch() throws Exception {
		Statement stmt = Connection().createStatement();
		Scanner scanner = new Scanner(System.in);
		System.out.println("[매니저 이름 검색]");
		System.out.println("매니저 성명>>");
		String mgr_name =scanner.next();
		String sql ="select rownum"
