...
<script>
	...
	function fn_submit() {
		var f = document.frm;
			if ( f.userid.value.length < 6 || f.userid.value.length > 12) {
				alert("아이디는 6~12자리 사이로 작성해주세요.");
				f.userid.focus();
				return false;
			}
			if ( f.userpw.value.length < 6 || f.userpw.value.length > 20) {
				alert("암호는 6~20자리 사이로 작성해주세요.");
				f.userpw.focus();
				return false;
			}
			if ( f.name.value == "") {
			...
			<form name = "frm" method = "post" action = "memberWriteSave.jsp">
				<table>
					...
					<tr>
						<th>*암호</th>
							<td>
								<input type="password" name = "userpw" class = "input1">
							</td>
					</tr>
					<tr>
						<th>*이름</th>
					...
					<tr>
						<th>이메일</th>
						<td>
							<input type="email" name = "eamil" class = "input1">
						</td>
					</tr>
					<tr>
						<th>홈페이지</th>
						<td>
							<input type="url" name = "homepage" class = "input1">
						</td>
					</tr>
				</table>
